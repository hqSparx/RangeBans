...
public class RBCommandHandler {
	...
	public RBIPFields checkIP(String ip)
	{
		...
		for(int i = 0; i < 4; i++){		
			...
			else if( split[i] == null || split[i].contentEquals("*") || split[i].contentEquals("")){
				min[i] = 0; max[i] = -128;	
				ip2 += "*";
			}			
	...
	public void ban(CommandSender sender, String[] args)
	{
     ...
     if (plugin.add(range)){
    	 try{
    	 plugin.saveLists();} catch (Exception e) {e.printStackTrace();}
	...
	public void checkip(CommandSender sender, String name)
	{
		...
		catch (Exception e) {if(ip == null || ip == "") plugin.strings.msg(sender, ("&cFailed to load player's IP: " + name));}
	}
